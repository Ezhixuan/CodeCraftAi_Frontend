    ...
    <a-card title="个人信息" :bordered="false">
      <a-spin :spinning="loading">
        ...
        <div class="avatar-section">
          <a-avatar v-if="userInfo.avatar" :size="120" :src="userInfo.avatar" class="user-avatar">
            <template #icon>
              <UserOutlined />
            </template>
          </a-avatar>
          <a-avatar v-else :src="userInfo.avatar" :size="120" :style="{ backgroundColor: '#1890ff' }">
            <template #icon>
        ...
        <a-form ref="formRef" :model="formData" :rules="rules" layout="vertical" class="user-form">
          <a-row :gutter="24">
            <a-col :span="12">
              <a-form-item label="昵称" name="name">
                <a-input v-model:value="formData.name" placeholder="请输入昵称" :disabled="!isEditing" />
              </a-form-item>
            </a-col>
            <a-col :span="12">
              <a-form-item label="邮箱" name="email">
                <a-input v-model:value="formData.email" placeholder="请输入邮箱" :disabled="!isEditing" />
              </a-form-item>
            </a-col>
          </a-row>
          ...
          <a-row :gutter="24">
            <a-col :span="12">
              <a-form-item label="头像链接" name="avatar">
                <a-input v-model:value="formData.avatar" placeholder="请输入头像链接" :disabled="!isEditing" />
              </a-form-item>
            </a-col>
            <a-col :span="12">
              <a-form-item label="创建时间">
                <a-input :value="formatDate(userInfo.createTime)" disabled placeholder="暂无数据" />
              </a-form-item>
            </a-col>
          </a-row>

          <a-form-item label="个人简介" name="profile">
            <a-textarea v-model:value="formData.profile" placeholder="请输入个人简介" :rows="4" :disabled="!isEditing" />
          </a-form-item>
          ...
          <template v-if="isEditing">
            ...
            <a-form-item label="新密码" name="password">
              <a-input-password v-model:value="formData.password" placeholder="如需修改密码请输入新密码" />
            </a-form-item>
          </template>
        </a-form>
...
import { UserOutlined, EditOutlined, SaveOutlined, CloseOutlined } from '@ant-design/icons-vue'
...
const fetchUserInfo = async () => {
  try {
    loading.value = true
    const result = (await getUserInfo()) as API.UserInfoCommonResVo
    ...
    // 更新全局用户状态
    loginUserStore.setLoginUser(result)
  } catch (error) {
...
const handleUpdate = async () => {
  try {
    ...
    // 调用更新接口
    const result = (await updateUserInfo(updateReq)) as API.UserInfoCommonResVo
